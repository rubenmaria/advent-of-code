...
def part_two(joltages, buttons):
    ...
    for i, (t, b) in enumerate(zip(joltages, buttons), 1):
        ...
        a, d, i = transfrom_matrix_in_row_echolon_form(a)
        print(a, i, d)
...
def transfrom_matrix_in_row_echolon_form(
    ...
    while pivot_row < matrix_row_size and pivot_column < matrix_column_size:
        ...
        if max_column_value < EPSILON:
            pivot_column += 1
            independent_variables.append(pivot_column)
            continue

        dependent_variables.append(pivot_column)
        ...
        for row in range(pivot_row + 1, matrix_row_size):
            coefficient = matrix[row][pivot_column] / matrix[pivot_row][pivot_column]
    ...
    independent_variables.extend([i for i in range(pivot_column, matrix_column_size)])
    return matrix, dependent_variables, independent_variables
