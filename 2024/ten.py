def trail_head_endings(map: list[list[int]], pos: tuple[int,int])-> set[tuple[int,int]]:
...
score_sum = sum(
    map(
        lambda x: len(trail_head_endings(hiking_map, x)),
        trail_heads_indecies
    )
)
rating_sum = sum(
    map(
        lambda x: trail_head_rating(hiking_map, x),
        trail_heads_indecies
    )
)
print(score_sum)
